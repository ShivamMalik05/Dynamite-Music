...
async function handleModal(interaction, client) {
  ...
  try {
    ...
    if (id === 'modal_block_title') {
      ...
      if (c.startsWith('#')) {
        data.color = parseInt(c.slice(1), 16);
      } else {
        data.color = parseInt(c, 16) || 0xFFFFFF;
      }
      ...
      if (id === 'modal_delete_block') data.blocks.splice(index, 1);
      else if (id === 'modal_move_up' && index > 0) [data.blocks[index - 1], data.blocks[index]] = [data.blocks[index], data.blocks[index - 1]];
      else if (id === 'modal_move_down' && index < data.blocks.length - 1) [data.blocks[index + 1], data.blocks[index]] = [data.blocks[index], data.blocks[index + 1]];
